...
# 3. DYNAMIC MODEL DISCOVERY (Ensures AI always works)
...
async def generate_gemini_response(prompt_text):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 404: CURRENT_MODEL_NAME = None
...
async def handle_booking(update: Update, user_text: str, rest_id: str):
    ...
    - valid (boolean): true if user specified Date/Time AND Number of People.
    - date (string): YYYY-MM-DD format (infer "tomorrow", "next sunday").
    - time (string): HH:MM format (24h).
    - guests (integer): number of people (default to 2 if ambiguous).
    - missing_info (string): if valid is false, what question to ask? (e.g. "What time?", "How many guests?").
    """
    
    ai_response = await generate_gemini_response(extraction_prompt)
    
    try:
        # Clean AI response (sometimes it adds ```json ... ```)
        clean_json = ai_response.replace("```json", "").replace("```", "").strip()
        details = json.loads(clean_json)
        
        if not details.get("valid"):
            # Ask the user for missing info
            await update.message.reply_text(details.get("missing_info", "I need the Date, Time, and Party Size to book."))
            return
        ...
        await update.message.reply_text("I couldn't understand the details. Please say something like: 'Book a table for 4 people tomorrow at 7pm'.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ROUTER: Check for Booking Keywords First
    keywords = ["book", "reserve", "table", "reservation", "booking", "slot", "seat", "place", "appointment"]
