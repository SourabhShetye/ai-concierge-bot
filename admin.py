"""
Restaurant Admin Dashboard — Streamlit  v6 (COMPLETE)
======================================================
Features:
  • Multi-restaurant management
  • Kitchen Display System (KDS) with auto-refresh
  • Live table billing
  • Menu manager (add/edit/delete/sold-out toggle)
  • Restaurant policies editor
  • Customer CRM insights (session-based tracking)
  • Table inventory management
  • Role-based access (admin/chef/manager)
"""
...
def to_dubai(utc_dt):
    """Convert UTC datetime to Dubai timezone"""
...
def build_menu_content(cat, item, price, desc):
    """Build menu item content string"""
    ...
    with tab1:
        ...
        try:
            bks = supabase.table("bookings").select("*")\
                ...
                .order("booking_time").execute().data
            
            if bks:
                cc1, cc2, cc3 = st.columns(3)
                cc1.metric("Total", len(bks))
                cc2.metric("Confirmed", sum(1 for b in bks if b["status"] == "confirmed"))
                        ...
                        if sel:
                            for bid in sel:
                                supabase.table("bookings").update({"status": "cancelled"})\
                                    .eq("id", bid).execute()
                            st.success(f"✅ Cancelled {len(sel)}")
                            st.rerun()
                        else:
                ...
                for order in orders:
                    ...
                    with st.container(border=True):
                        ...
                        if mod_status == "requested" and pending_blob:
                            ...
                            st.warning(
                                f"✏️ **MOD REQUEST — Order #{oid}**\n\n"
                                f"Table **{order['table_number']}** remove: **{removed}**\n\n"
                                ...
                                if st.button("✅ Approve", key=f"amod_{oid}", use_container_width=True, type="primary"):
                                    try:
                                        ...
                                        if order.get("chat_id"):
                                            send_telegram(order["chat_id"], msg)
                            ...
                            with mc2:
                                if st.button("❌ Reject", key=f"rmod_{oid}", use_container_width=True):
                                    try:
                                        ...
                                        if order.get("chat_id"):
                                            send_telegram(order["chat_id"], f"❌ *Change rejected — Order #{oid}*\nOriginal: {order['items']}")
                        ...
                        if order.get("cancellation_status") == "requested":
                            ...
                            with cc1:
                                if st.button("✅ Approve Cancel", key=f"acan_{oid}", use_container_width=True, type="primary"):
                                    try:
                                        ...
                                        if order.get("chat_id"):
                                            send_telegram(order["chat_id"], f"✅ *Order #{oid} cancelled* — approved.")
                            ...
                            with cc2:
                                if st.button("❌ Reject", key=f"rcan_{oid}", use_container_width=True):
                                    try:
                                        ...
                                        if order.get("chat_id"):
                                            send_telegram(order["chat_id"], f"❌ *Cancellation rejected — Order #{oid}.*")
                            ...
                            if st.button("✅ Mark Ready", key=f"ready_{oid}", use_container_width=True, type="primary"):
                                try:
                                    ...
                                    if order.get("chat_id"):
                                        send_telegram(order["chat_id"], f"🍽️ *Order #{oid} ready!* (Table {order['table_number']})")
...
if user_role in ["admin", "manager"]:
    with tab3:
        ...
        try:
            live = supabase.table("orders").select("*")\
                .eq("restaurant_id", cur_rid)\
                .neq("status", "paid")\
                .neq("status", "cancelled").execute().data
                    ...
                    with st.container(border=True):
                        ...
                        if st.button("💳 Close Table & Payment", key=f"pay_{tn}", use_container_width=True, type="primary"):
                            try:
                                ...
                                # Set feedback state for each session
                                session_ids = set()
                                for o in data["orders"]:
                                ...
                                st.success(f"✅ Table {tn} closed" + (" & feedback sent" if ok else ""))
                                st.rerun()
                            except Exception as ex:
                                st.error(f"{ex}")
            else:
...
if user_role in ["admin", "manager"]:
    with tab4:
        ...
        try:
            menu_items = supabase.table("menu_items").select("id,content,sold_out")\
                .eq("restaurant_id", cur_rid).execute().data or []
        except Exception as ex:
            st.error(f"{ex}")
            menu_items = []
        ...
        with st.expander("➕ Add New Item", expanded=False):
            with st.form("add_item", clear_on_submit=True):
                ...
                n_name = c2.text_input("Item Name *", placeholder="Burger")
                
                d1, d2 = st.columns(2)
                n_price = d1.text_input("Price *", placeholder="$18")
                n_desc = d2.text_input("Description")
                
                if st.form_submit_button("➕ Add", type="primary", use_container_width=True):
                    if not all([n_cat.strip(), n_name.strip(), n_price.strip()]):
                        st.error("Category, Name, Price required.")
                    else:
        ...
        st.markdown("---")
        st.subheader(f"📋 Menu ({len(menu_items)} items)")
        ...
        if not menu_items:
            ...
            for row in menu_items:
                ...
                grouped.setdefault(cat, []).append({"id": row["id"], "p": p, "sold_out": row.get("sold_out", False)})
                ...
                for entry in items:
                    ...
                    with st.container(border=True):
                        ...
                        if dc3.button(toggle_text, key=f"toggle_{rid_row}", use_container_width=True):
                            try:
                                supabase.table("menu_items").update({"sold_out": not sold_out})\
                        ...
                        if st.session_state.get(ekey):
                            with st.form(f"ef_{rid_row}"):
                                ...
                                e_name = ec2.text_input("Name", value=p["item"])
                                
                                fc1, fc2 = st.columns(2)
                                e_price = fc1.text_input("Price", value=p["price"])
                                e_desc = fc2.text_input("Desc", value=p["description"])
...
if user_role in ["admin", "manager"]:
    with tab5:
        ...
        try:
            pol = supabase.table("restaurant_policies").select("id,policy_text")\
                .eq("restaurant_id", cur_rid).limit(1).execute()
            if pol.data:
                existing = pol.data[0].get("policy_text", "")
                pol_id = pol.data[0].get("id")
        except Exception as ex:
            st.warning(f"Could not load policy: {ex}")
        ...
        with col_tips:
            ...
            st.markdown("🔑 WiFi\n🅿️ Parking\n♿ Accessibility\n🕐 Hours\n🌿 Dietary\n💳 Payments\n📞 Contact")
            if existing:
                st.metric("Words", len(existing.split()))
...
if user_role in ["admin", "manager"]:
    with tab6:
        ...
        try:
            ...
            st.error(f"Error loading sessions: {ex}")
            all_sessions = []
        ...
        if not display:
            ...
            for s in display:
                ...
                risk_icon = "🔴" if "Churn Risk" in tags else (
                    "👑" if "VIP" in tags else (
                        "🌟" if "Big Spender" in tags else (
                            "😊" if "Frequent Diner" in tags else "⚪"
                        )
                    )
                )
...
if user_role in ["admin", "manager"]:
    with tab7:
        ...
        try:
            ...
            st.error(f"Error loading inventory: {ex}")
            inv_data = []
        ...
        with st.form("add_inv", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Save", type="primary", use_container_width=True):
                try:
                    ...
                    st.success(f"✅ Saved: {int(new_count)}x {int(new_cap)}-seater tables")
                    st.rerun()
                except Exception as ex:
                    st.error(f"Error: {ex}")
